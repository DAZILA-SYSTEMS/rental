		...
		.then((payment) => {
			...
			House.findOne({
				...
				.then((house) => {
					...
					Invoice.findOne({
						where: {
							houseLinker: house.dataValues.linker,
						},
						order: [["linker", "DESC"]],
					})
						.then((invoice) => {
							...
							//create an payment
							Payment.create({
								name: invoice.dataValues.billLinker,
								details: `Payment for ${invoice.dataValues.billLinker}`,
								amount: req.body.TransAmount,
								code: req.body.TransID,
								mode: "Mpesa-Online",
								credLinker: "Mpesa-Online",
								invoiceLinker: invoice.dataValues.linker,
								instLinker: req.params.instLinker,
								live: 1,
								linker: Date.now(),
								trace: Date.now(),
...
const CompletePush = (req, amount, code) => {
	Payment.findOne({
		...
		.then((payment) => {
			...
			Invoice.findOne({
				where: {
					linker: req.params.invoiceLinker,
				},
			})
				.then((invoice) => {
					if (!invoice) {
